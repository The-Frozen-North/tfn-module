...
void main()
{
    ...
    RemoveSpellEffects(SPELL_GREATER_MAGIC_FANG, spell.Caster, oTarget);
    RemoveSpellEffects(spell.Id, spell.Caster, oTarget);

    effect eVis = EffectVisualEffect(VFX_IMP_HOLY_AID);

    effect eAttack = EffectAttackIncrease(1);
    effect eDamage = EffectDamageIncrease(1, DAMAGE_TYPE_MAGICAL);
    effect eReduction = EffectDamageReduction(1, DAMAGE_POWER_PLUS_ONE); // * doing this because
                                                                     // * it creates a true
                                                                     // * enhancement bonus

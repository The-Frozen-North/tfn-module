    ...
    while(GetIsObjectValid(oTarget))
    {
        if(!GetIsReactionTypeFriendly(oTarget, spell.Caster))
        {
            for(i=1; i <= nExtraMissiles + nRemainder; i++)
            {
                ...
                DelayCommand(fDelay, ActionCastFakeSpellAtObject(SPELL_TRAP_ARROW, oTarget, PROJECTILE_PATH_TYPE_HOMING));
                nDamage = GetSavingThrowAdjustedDamage(d6(spell.Level),oTarget,spell.DC,SAVING_THROW_REFLEX,SAVING_THROW_TYPE_TRAP);
                if(nDamage > 0)
                {

...
void main()
{
    ...
    object oTarget = FIX_GetFirstObjectInShape(SHAPE_SPELLCONE, 14.0, GetSpellTargetLocation(), TRUE);
    while(GetIsObjectValid(oTarget))
    {
        if(oTarget != OBJECT_SELF && spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            nDamStrike = d6(nDamage);
            //Adjust the damage based on the Reflex Save, Evasion and Improved Evasion.
            nDamStrike = GetSavingThrowAdjustedDamage(nDamStrike, oTarget, nDC, SAVING_THROW_REFLEX, SAVING_THROW_TYPE_COLD, OBJECT_SELF);
            if (nDamStrike > 0)
            {
                //Set Damage
        ...
        oTarget = FIX_GetNextObjectInShape(SHAPE_SPELLCONE, 14.0, GetSpellTargetLocation(), TRUE);
    }
}

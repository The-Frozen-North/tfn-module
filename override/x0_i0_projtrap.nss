                           ...
                           int nProjectilePath = PROJECTILE_PATH_TYPE_DEFAULT)
{
    ...
    //using CP's override feature as caster level, metamagic and DC uses default values when cheat-casting
    SetLocalInt(oTrapOrigin,"SPECIAL_ABILITY_CASTER_LEVEL_OVERRIDE",nCasterLevel);
    int nDC = GetLocalInt(OBJECT_SELF,"DC");
    if(nDC > 0) SetLocalInt(oTrapOrigin,"SPECIAL_ABILITY_DC_OVERRIDE",nDC);
    int nMeta = GetLocalInt(OBJECT_SELF,"Metamagic");
    if(nMeta > 0) SetLocalInt(oTrapOrigin,"SPECIAL_ABILITY_METAMAGIC_OVERRIDE",nMeta);
    //override to ignore mantle/spell immunity/SR
    SetLocalInt(oTrapOrigin,"DISABLE_RESIST_SPELL_CHECK",TRUE);

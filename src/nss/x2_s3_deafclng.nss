...
void main()
{
   ...
   if (GetIsObjectValid(oItem))
   {
      ...
      if (!GetHasSpellEffect(GetSpellId(),oSpellTarget) && !GetHasEffect(EFFECT_TYPE_DEAF,oSpellTarget))
      {
          //if (!ResistSpell(oSpellOrigin,oSpellTarget) == 0)  //1.70: removed resist spell returns always -1
          int nDC= GetCasterLevel(oSpellOrigin)+10;
          if (!MySavingThrow(SAVING_THROW_FORT,oSpellTarget, nDC, SAVING_THROW_TYPE_SONIC, oSpellOrigin))
          {

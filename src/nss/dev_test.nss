...
int GetIdentifiedGoldValue(object oItem)
{
...
struct results main2()
{
    struct results out;
// this should never trigger on a PC, nunless it's DM-possessed
   //if (GetIsPC(OBJECT_SELF) == TRUE && GetIsDMPossessed(OBJECT_SELF) == FALSE) return out;

// if it is tagged for no credit, also stop
   //if (GetLocalInt(OBJECT_SELF, "no_credit") == 1) return out;
...
// stop if there are 0 players around
   if (Party.TotalSize == 0) return out;
...
// if destroyed, the container is a loot bag
        if (bDestroyed)
        {
            oContainer = GetLocalObject(OBJECT_SELF, "container");
            if (!GetIsObjectValid(oContainer))
            {
                oContainer = CreateObject(OBJECT_TYPE_PLACEABLE, "_loot_container", GetLocation(OBJECT_SELF));
                SetLocalObject(OBJECT_SELF, "container", oPersonalLoot);
            }
...
// If there's a quest on the object, add a quest item to their personal if this player is eligible
        ...
        if (bNoTreasure == FALSE)
        {
           ...
           SetLocalInt(oPersonalLoot, "area_cr", GetLocalInt(oContainer, "area_cr"));
			...
			if (nNth == nItem1)
			{
				oItem = GenerateLoot(oPersonalLoot);
                out.nPlayerValue += GetIdentifiedGoldValue(oItem);
                DestroyObject(oItem);
				out.nPlayer++;
			}
			if (nNth == nItem2)
...
// assumed to be out of bounds (henchman)
				if (Party.PlayerSize+nNth == nItem1)
               {
				   ...
				   out.nHench++;
				   out.nHenchValue += GetIdentifiedGoldValue(oItem1);
               }
...
void main()
{
    int nRemaining = GetLocalInt(GetModule(), "testremaining");
    if (nRemaining <= 0)
    {
        SetLocalInt(GetModule(), "testplayer", 0);
        SetLocalInt(GetModule(), "testhench", 0);
	    SetLocalInt(GetModule(), "testplayervalue", 0);
        SetLocalInt(GetModule(), "testhenchvalue", 0);
        nRemaining = 5000;
    }
    if (GetIsPC(OBJECT_SELF))
    {
        object oTest = GetNearestCreature(CREATURE_TYPE_IS_ALIVE, FALSE, OBJECT_SELF);
        FloatingTextStringOnCreature("Run on: " + GetName(oTest), GetFirstPC());
        DelayCommand(6.0, ExecuteScript("dev_test", oTest));
        return;
    }
    NWNX_Util_SetInstructionLimit(52428888);
	SetLocalInt(OBJECT_SELF, "cr", 9);
	SetLocalInt(OBJECT_SELF, "area_cr", 9);
	int i;
	int nPlayer = GetLocalInt(GetModule(), "testplayer");
	int nHench = GetLocalInt(GetModule(), "testhench");
	int nPlayerValue = GetLocalInt(GetModule(), "testplayervalue");
	int nHenchValue = GetLocalInt(GetModule(), "testhenchvalue");
	for (i=0; i<10; i++)
	{
		struct results out = main2();
		nPlayer += out.nPlayer;
		nHench += out.nHench;
		nPlayerValue += out.nPlayerValue;
		nHenchValue += out.nHenchValue;
		
	}
    SetLocalInt(GetModule(), "testplayer", nPlayer);
    SetLocalInt(GetModule(), "testhench", nHench);
    SetLocalInt(GetModule(), "testplayervalue", nPlayerValue);
    SetLocalInt(GetModule(), "testhenchvalue", nHenchValue);
    
    SendMessageToPC(GetFirstPC(), "nPlayer = " + IntToString(nPlayer));
    SendMessageToPC(GetFirstPC(), "nHench = " + IntToString(nHench));
    SendMessageToPC(GetFirstPC(), "nPlayerValue = " + IntToString(nPlayerValue));
    SendMessageToPC(GetFirstPC(), "nHenchValue = " + IntToString(nHenchValue));
    SendMessageToPC(GetFirstPC(), "remaining = " + IntToString(nRemaining - 10));
    nRemaining -= 10;
    if (nRemaining <= 0)
    {
        DeleteLocalInt(GetModule(), "testremaining");
    }
    else
    {

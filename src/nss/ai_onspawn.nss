...
// cast defensive buffs instantly
    ...
    // *
    if (!GetPlotFlag(OBJECT_SELF) && !GetImmortal(OBJECT_SELF) && GetSkillRank(SKILL_HIDE, OBJECT_SELF, TRUE) > 0) SetSpawnInCondition(NW_FLAG_STEALTH);
...
// if they have a range weapon equipped, they are considered a range attacker
    if (GetWeaponRanged(GetItemInSlot(INVENTORY_SLOT_RIGHTHAND))) SetCombatCondition(X0_COMBAT_FLAG_RANGED);
    ...
    // * attackers.
    if (GetClassByPosition(1) == CLASS_TYPE_COMMONER) SetCombatCondition(X0_COMBAT_FLAG_COWARDLY);

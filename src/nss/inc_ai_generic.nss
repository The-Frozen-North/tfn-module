...
{
    // Taunt the enemy!
    // pok - we won't use any voice chat here, we'll play it during combat round determination
    /*
    ...
    */
    ...
    // pok - we won't check for polymorph. in fact, only the basic weapon switch will run
    ...
    // INTELLIGENT WEAPON/SHIELD CHOOSING AND PICKING UP DISARMED HAS BEEN REMOVED - pok

    // There was a function to equip a shield if no range weapon or in-effective weapon
    // we can safely remove this, because I don't put a shield with a 2h or range usually. - pok
...
/*  Ability: Strength.
    ...
    // Ranged weapon?
    // pok - let's not check for polymorph
    if(iRangedAttack && ValidFeats)
    {
        // RANGED: Add dexterity
        iOurHit += GetAbilityModifier(ABILITY_DEXTERITY);

        // pok - we can't use power attacks with throwing axes
    ...
    // decent skill.
    else if(!iRangedAttack && !GetSpawnInCondition(AI_FLAG_OTHER_COMBAT_NO_PARRYING, AI_OTHER_COMBAT_MASTER) &&
    ...
    else if(!iRangedAttack)
    {
        ...
        int iTargetWeaponSize, iTargetCreatureSize, iTargetCreatureRace, iOurWeaponSize,
            iCanUseMonks, iAddingModifier, iMonkLevels;
        // Monk levels
        ...
        // pok - any smite will be used, because cha will be added to attack roll regardless
        ...
        if((GlobalOurPercentHP <= i20) ||
           (GlobalMeleeAttackers && GlobalTotalAllies >= i2))
        {
            // +10 AC, -10 BAB
        ...
        // no check, use quivering palm if you got it - pok
...
int AI_AttemptConcentrationCheck(object oTarget)
{
    // we won't check for epic feats
    // defensive casting will not be used - pok
...
int AI_ActionCastSpell(int iSpellID, int nTalent = 0, object oTarget = OBJECT_SELF, int iRequirement = 0, int iLocation = FALSE, int iItemTalentValue = -1, int iPotionTalentValue = -1)
{
    // we won't check for an ethereal (greater sanctuary) enemy - pok
    // object seen is probably enough
    if (!iLocation && !GetObjectSeen(oTarget))
        return FALSE;
...
//          (iLocation || GetObjectSeen(oTarget)))
        {
            ...
            // maybe seen check is enough - pok
...
*/
    ...
    // pok - weapon buff spells are removed because i fast buff em
...
//        _db("Ethereal targets check: " + IntToString(iCheck));
        ...
        switch(d4())
        {
            case 1: sShout = "I thought I heard something..."; break;
